   ...
   const openModal = async (type: "default" | "callback", name: string) => {
    ...
    return (
        ...
        <div className="w-full md:mx-auto md:py-10 2xl:px-25 xl:px-20 lg:px-10 p-5 bg-[#1f2227]">
            ...
            } className="cursor-pointer text-white text-center font-semibold border border-solid border-white bg-[#1f2227] hover:bg-white hover:text-[#1f2227] text-sm h-10 px-4 rounded-lg flex items-center">
                     Get in Touch
                </button>
            </div>
            </div>
...
/>
            ...
            <div>
                <ul className="relative reulist">
                    {reasons.map((reason) => (
                        <li key={reason.slno} className="md:mb-4 mb-5 text-white z-20 relative text-center b-scale__item">
